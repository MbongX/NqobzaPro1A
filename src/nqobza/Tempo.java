...
public class Tempo {
    //Base class(Mother Class) with all the defined variables
    ...
    //Derived Class(daughter class) from The base class(mother class)
    public class registerUser extends initDefs // handles all the registration
    {

    }

    // handles the authentication of user into system
    public static class loginUser extends initDefs {
        //Constructors
        ...
        void usernameAndPassword() {
            ...
            if (User_name.length() <= 5 && User_name.contains("_")) {  // if statement is what i used to set the user names name lenght to be equals to 5 or less
                ...
                System.out.println("Username has no underscore .Could not Capture User_name:");// then the console will print to the user could not be able to capture user name
